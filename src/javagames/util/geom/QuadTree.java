...
public class QuadTree extends BoundingBox
{
	class AABB extends BoundingBox
	{
		...
		public AABB combine(BoundingBox other)
		{
			return (AABB)combine(other);
		}
	}
	...
	public void insert(AABB e)
	{
		...
		while (node.isLeaf() == false)
		{
			...
			AABB combinedAABB = node.aabb.combine(e);
			...
			if (child1.isLeaf())
			{
				AABB aabb = e.combine(child1.aabb);
			...
			if (child2.isLeaf())
			{
				AABB aabb = e.combine(child2.aabb);
	...
	public class QuadQuery implements Iterable<BoundingBox>
	{
		private BoundingBox area;
		
		public QuadQuery(BoundingBox area)
		{
			this.area = area;
		}
	...
	private class Tree2Diterator implements  Iterator<BoundingBox>
	{
		private BoundingBox area;
		private Stack<TreeNode> stack;
		private BoundingBox ret;
		
		public Tree2Diterator(BoundingBox area)
		{
			this.area = area;
		...
		public boolean hasNext()
		{
			...
			while (stack.size() > 0)
			{
				...
				if (n.aabb.intersects(area))
				{
					if (n.isLeaf())
					{
						ret = (BoundingBox) n.aabb;
						return true;
					} else
					{
						stack.push(n.child1);
	...
	public QuadQuery query(AABB treeAABB)
	{
		return new QuadQuery(treeAABB);
	}
}